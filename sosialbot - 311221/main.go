	...
	if update.CallbackQuery != nil {
		...
		*/
		...
		if update.CallbackQuery.Data[:i] == "good" {
			err := db.QueryRow("update requests set feedback=$1 where reqnumber=$2 and feedback is null returning id::numeric", "Yaxşı", reqnumber).Scan(&reqId)
		...
		if update.CallbackQuery.Data[:i] == "middle" {
			err := db.QueryRow("update requests set feedback=$1 where reqnumber=$2 and feedback is null returning id::numeric", "Orta", reqnumber).Scan(&reqId)
			if err != nil && err != sql.ErrNoRows {
				panic(err)
			}
			if reqId > 0 {
...
func requestsDoneGetHandler(w http.ResponseWriter, r *http.Request) {
 ...
 where reqnumber=$1 and status=1 returning reqfrom::numeric `, reqnumber).Scan(&telegramid)
	if err != nil && err != sql.ErrNoRows {
		panic(err)
	}
...
func userRequestSaveGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	var rows, err = db.Query(`select coalesce(s.service_name,'') as service_name,
									...
									where  s.id=$1 and  s2.data_driven=1 
									 order by s2.order_num`, servicesrequestsid)
	if err != nil {
		panic(err)
	}
